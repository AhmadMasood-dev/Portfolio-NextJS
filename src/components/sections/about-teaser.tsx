...
const facts = [
  ...
  { icon: Award,         label: 'Recognition', value: 'Microsoft Learn Student Ambassador' },
]

export function AboutTeaser() {
  const shouldReduce = useReducedMotion()

  /* ─── 3D spring tilt for Quick Facts card ─── */
  ...
  const handleMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    rawRY.set(((e.clientX - r.left  - r.width  / 2) / (r.width  / 2)) * 8)
    rawRX.set(((r.top + r.height / 2 - e.clientY)   / (r.height / 2)) * 8)
  }
    ...
    <section className="py-24 md:py-32">
      <div className="mx-auto max-w-6xl px-6">
        <div className="grid grid-cols-1 gap-12 md:grid-cols-2 md:items-center">
          ...
          {/* Right — 3D tilt Quick Facts card */}
          <SectionReveal delay={0.15}>
            <div style={{ perspective: '900px' }}>
              <motion.div
                ...
                className="rounded-[1.5rem] bg-black/[0.03] p-1.5 ring-1 ring-black/6 transition-shadow duration-300 hover:shadow-2xl hover:shadow-black/8 cursor-default"
              >
